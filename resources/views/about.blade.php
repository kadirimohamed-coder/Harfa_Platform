@extends('layouts.app')
@section('title', 'À propos — Harfa.ma')
@section('content')
<div class="container py-4">
        ...
        <ul style="list-style:none;padding:0;margin:0">
          @foreach([
            ...
            <li class="d-flex align-items-center gap-3 mb-3">
              ...
              <span style="font-size:15px;color:#334155">{{ $pt }}</span>
            </li>
          @endforeach
        </ul>
      </div>

      {{-- Features grid (bdalna stats fake) --}}
      <div class="col-lg-6">
        <div class="row g-3">
          @foreach([
            ...
            <div class="col-6">
              <div style="background:white;border:1.5px solid #e2e8f0;border-radius:16px;padding:22px 16px;text-align:center;height:100%">
                <div style="font-size:32px;margin-bottom:10px">{{ $s[0] }}</div>
                <div style="font-size:15px;font-weight:700;color:#0f172a;margin-bottom:6px">{{ $s[1] }}</div>
                <div style="font-size:12px;color:#64748b;line-height:1.5">{{ $s[2] }}</div>
              </div>
            </div>
          @endforeach
        </div>
      ...
      @foreach([
        ...
        <div class="col-md-6 col-lg-3" data-aos="fade-up" data-aos-delay="{{ $loop->index * 80 }}">
          <div class="step-card h-100">
            <div class="step-icon" style="background:{{ $v[3] }};color:{{ $v[4] }}">
              <i class="bi {{ $v[0] }}"></i>
            </div>
            <h5>{{ $v[1] }}</h5>
            <p>{{ $v[2] }}</p>
  ...
  {{-- Pourquoi Harfa.ma (badalna Notre équipe) --}}
  <section class="mb-5" data-aos="fade-up">
    <div class="text-center mb-4">
    ...
    <div class="row g-4">
      @foreach([
        ...
        <div class="col-md-6 col-lg-4" data-aos="fade-up" data-aos-delay="{{ $loop->index * 60 }}">
          <div class="why-card">
            <div class="why-icon" style="background:{{ $r[3] }};color:{{ $r[4] }}">
              <i class="bi {{ $r[0] }}"></i>
            </div>
            <div>
              <div class="why-title">{{ $r[1] }}</div>
              <div class="why-desc">{{ $r[2] }}</div>
            </div>
          </div>
        </div>
      @endforeach
  ...
  <div class="cta-section text-center" data-aos="fade-up">
    ...
    <div class="d-flex justify-content-center gap-3 flex-wrap">
      <a href="{{ route('register') }}" class="btn-harfa">
