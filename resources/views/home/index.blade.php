@extends('layouts.app')
@section('title', 'Harfa.ma — Trouvez un artisan de confiance')
...
<div class="container py-4">
  ...
  <section class="hero-section text-center" data-aos="fade-down">
    ...
    {{-- Search card --}}
    <form action="{{ route('craftsmen.index') }}" method="GET" class="hero-search-wrapper">
    ...
    <div class="hero-tags">
      @foreach($categories->take(6) as $c)
        <a href="{{ route('categories.show', $c) }}" style="text-decoration:none">
          <span>{{ $c->name }}</span>
        </a>
      @endforeach
    </div>
    ...
    {{-- Trust bar
    <div class="trust-bar mt-5">
      <div class="trust-item">
        <div class="trust-num">500+</div>
        <div class="trust-label">Artisans vérifiés</div>
      </div>
      <div class="trust-item">
        <div class="trust-num">2 000+</div>
        <div class="trust-label">Clients satisfaits</div>
      </div>
      <div class="trust-item">
        <div class="trust-num">4.8★</div>
        <div class="trust-label">Note moyenne</div>
      </div>
      <div class="trust-item">
        <div class="trust-num">24h</div>
        <div class="trust-label">Délai de réponse</div>
      </div>
    </div> --}}
  ...
  <section class="mb-5" data-aos="fade-up">
    <div class="section-header">
      ...
      <a href="{{ route('craftsmen.index') }}" class="section-link">
        Voir tout <i class="bi bi-arrow-right"></i>
      </a>
    </div>
    <div class="row g-3">
      @foreach($categories as $c)
        <div class="col-6 col-md-3 col-lg-2" data-aos="fade-up" data-aos-delay="{{ $loop->index * 60 }}">
          <a href="{{ route('categories.show', $c) }}" class="category-card">
            <div class="cat-icon">
              <i class="bi {{ $c->icon ?? 'bi-tools' }}"></i>
            </div>
            <h5>{{ $c->name }}</h5>
            <span>{{ $c->craftsmen_count ?? '' }} artisans</span>
          </a>
        </div>
      @endforeach
    </div>
    ...
    <div class="row g-4">
      @forelse($top as $cm)
        <div class="col-md-4" data-aos="fade-up" data-aos-delay="{{ $loop->index * 80 }}">
          <a href="{{ route('craftsmen.show', $cm) }}" class="text-decoration-none">
            <div class="artisan-card">
              ...
              <div class="artisan-body">
                ...
                <h5 class="mb-1 fw-bold" style="font-size:16px;color:#0f172a">{{ $cm->user->name }}</h5>
                <p class="mb-2" style="font-size:13px;color:#64748b">
                  @if($cm->categories->count())
                    {{ $cm->categories->first()->name }}
                  @endif
                  @if($cm->user->city) · {{ $cm->user->city }} @endif
                </p>
                <div class="mb-2 d-flex justify-content-center align-items-center gap-1">
                  @for($i = 1; $i <= 5; $i++)
                    <i class="bi bi-star{{ $i <= round($cm->reviews_avg_rating ?? 0) ? '-fill text-warning' : ' text-muted' }}" style="font-size:14px"></i>
                  @endfor
